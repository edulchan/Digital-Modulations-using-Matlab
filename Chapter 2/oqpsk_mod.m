...
%Function to modulate an incoming binary stream using OQPSK (bandpass)
%a - input binary data stream (0's and 1's) to modulate
%fc - carrier frequency in Hertz
%OF - oversampling factor
%s - O-QPSK modulated signal with carrier
...
I = [I(:) ; zeros(L/2,1)].'; %padding at end to equal length of Q
Q = [zeros(L/2,1); Q(:)].'; %delay by Tb (delay by L/2)

fs = OF*fc; %sampling frequency
t=(0:1:length(I)-1)/fs; %time base
iChannel = I.*cos(2*pi*fc*t);qChannel = -Q.*sin(2*pi*fc*t);
s = iChannel + qChannel; %O-QPSK modulated baseband signal
...
if doPlot==1%Waveforms at the transmitter
    ...
    subplot(3,2,2);plot(t,iChannel,'r');%I(t) with carrier
    xlabel('t'); ylabel('I(t)-with carrier');xlim([0,10*L/fs]);
    subplot(3,2,3);plot(t,Q);%baseband waveform on Q arm zoomed to first few symbols
    ...
    figure;plot(I,Q);axis([-1.5 1.5 -1.5 1.5]) %constellation plot at transmitter
end